...
task CountUniqueVariantIdsInOverlap {
  input {
    ...
    Int cpu = 1
    Int memory_mb = 16000
    Int disk_size_gb = 3 * ceil(size(variant_ids_1, "GiB") + size(variant_ids_2, "GiB")) + 20
  }
  ...
  command <<<
    ...
    # The reference panel's id list can be large (sequence-resolved SV alleles -> long lines), and
    # `sort` needs room for its temp runs + the sorted copy. Keep temps on the (widened) local disk,
    # gzip them to shrink the footprint, and use a big in-RAM buffer to minimize spilling.
    td="$(pwd)"; export TMPDIR="$td"

    sort -S ~{command_mem}M --compress-program=gzip -T "$td" ~{variant_ids_1} > variant_ids_1_sorted
    sort -S ~{command_mem}M --compress-program=gzip -T "$td" ~{variant_ids_2} > variant_ids_2_sorted
...
task Impute {
  input {
    ...
    Boolean impute_with_genotype_probabilities = true   # emit FORMAT/GP (posterior genotype probabilities)
  ...
  command <<<
    ...
    ~{if impute_with_genotype_probabilities then "gp=true" else ""} \
    nthreads=~{cpu} \
    seed=-99999
...
# ---------------------------------------------------------------------------
# Pop the imputed bubbles back to their constituent variants (collision
# resolution). Reused from the GLIMPSE2/IMPUTE5 siblings; engine-agnostic but
# needs FORMAT/GP, so the Beagle run must use impute_with_genotype_probabilities.
task PopAndMarginalizeCollisions {
  input {
    ...
    File? cargo_toml                             # Cargo.toml for the Rust build
    File? pop_glimpse2_binary                    # optional prebuilt binary (skips cargo build)
    String? region                               # e.g. "chr1"; omit to pop the whole VCF
    String basename
    String docker                                # needs bcftools + cargo (Rust toolchain)

    Int cpu = 4
    Int memory_mb = 16000
    Int disk_size_gb = ceil(3*(size(posteriors_vcf, "GiB") + size(panel_bubble_split_sites_only_vcf, "GiB") + size(panel_id_split_vcf_gz, "GiB"))) + 20
    Int preemptible = 2
  }
  command <<<
    set -euox pipefail

    # Obtain the pop-glimpse2 binary: use a prebuilt one if supplied, else build it from the
    # Rust source + Cargo.toml (cargo build --release). Layout mirrors the panel-creation WDL:
    # src/bin/pop-glimpse2.rs + Cargo.toml (package name "pop-glimpse2") -> target/release/pop-glimpse2.
    if [ -n "~{pop_glimpse2_binary}" ]; then
        POP_BIN="~{pop_glimpse2_binary}"
        chmod +x "$POP_BIN"
    elif [ -n "~{pop_glimpse2_script}" ] && [ -n "~{cargo_toml}" ]; then
        mkdir -p pop-glimpse2/src/bin
        cp ~{pop_glimpse2_script} pop-glimpse2/src/bin/pop-glimpse2.rs
        cp ~{cargo_toml}          pop-glimpse2/Cargo.toml
        ( cd pop-glimpse2 && cargo build --release )
        POP_BIN="./pop-glimpse2/target/release/pop-glimpse2"
    else
        echo "ERROR: provide pop_glimpse2_binary, or both pop_glimpse2_script (.rs) and cargo_toml" >&2
        exit 1
    fi
