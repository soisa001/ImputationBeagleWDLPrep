...
import "ImputationBeagleStructs.wdl" as structs
import "ImputationTasks.wdl" as tasks
import "ImputationBeagleTasks.wdl" as beagleTasks

workflow ImputeBeagleWithPop {
  ...
  input {
    ...
    Boolean impute_with_genotype_probabilities = true   # emit FORMAT/GP in the imputed VCF

    # --- bubble-pop / collision postprocess (OPTIONAL) -------------------------
    # Provide all of these to pop the imputed bubbles back to constituent variants.
    # Uses FORMAT/GP, so it requires impute_with_genotype_probabilities = true.
    ...
    File? pop_glimpse2_script                           # pop-glimpse2 Rust source (src/bin/pop-glimpse2.rs)
    File? pop_glimpse2_cargo_toml                       # Cargo.toml for the Rust build
    File? pop_glimpse2_binary                           # optional prebuilt binary (skips cargo build)
    String? pop_region                                  # e.g. "chr1"; omit to pop the whole imputed VCF
    String pop_docker = "us.gcr.io/broad-dsde-methods/slee/lrma-aou2-panel-creation-rust:v1"   # needs bcftools + cargo (Rust)
  }
      ...
      scatter (j in range(num_sample_chunks)) {
        ...
        call beagleTasks.Impute {
          input:
            ...
            impute_with_genotype_probabilities = impute_with_genotype_probabilities,
            cpu = beagle_cpu,
  ...
  # --- pop bubbles back to constituent variants (OPTIONAL) --------------------
  # Runs only when all the pop inputs are provided; otherwise the imputed VCF is
  # the final output unchanged. Requires impute_with_genotype_probabilities=true.
  if (defined(panel_id_split_vcf_gz) && defined(panel_bubble_split_sites_only_vcf)
      && (defined(pop_glimpse2_binary) || (defined(pop_glimpse2_script) && defined(pop_glimpse2_cargo_toml)))) {
    call beagleTasks.PopAndMarginalizeCollisions {
      input:
        posteriors_vcf       = CreateIndexForGatheredVcf.output_vcf,
        posteriors_vcf_index = CreateIndexForGatheredVcf.output_vcf_index,
        panel_bubble_split_sites_only_vcf     = select_first([panel_bubble_split_sites_only_vcf]),
        panel_bubble_split_sites_only_vcf_idx = select_first([panel_bubble_split_sites_only_vcf_idx]),
        panel_id_split_vcf_gz     = select_first([panel_id_split_vcf_gz]),
        panel_id_split_vcf_gz_tbi = select_first([panel_id_split_vcf_gz_tbi]),
        pop_glimpse2_script = pop_glimpse2_script,
        cargo_toml          = pop_glimpse2_cargo_toml,
        pop_glimpse2_binary = pop_glimpse2_binary,
        region              = pop_region,
        basename            = output_basename + ".imputed.popped",
        docker              = pop_docker
    }
  }
  ...
  output {
    ...
    File? imputed_popped_vcf = PopAndMarginalizeCollisions.output_vcf
    File? imputed_popped_vcf_index = PopAndMarginalizeCollisions.output_vcf_index
